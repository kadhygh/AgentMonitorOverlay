...
export function useFocusCommands() {
  ...
  useEffect(() => {
    ...
    void listen<{ requestId: string; ok: boolean; message: string }>("amo-focus-session-command-result", event => {
      ...
      setStatus(previous => ({ ...previous, [request.cardId]: { pending: false, message: event.payload.ok ? `Request delegated. ${event.payload.message || "Check the CLI window for the result."}` : event.payload.message || "The request could not be delegated." } }));
  ...
  async function send(card: FocusCardView, action: "activate" | "resume") {
    if (!ready || !card.session || !commandAvailable(card, action) || status[card.cardId]?.pending) return;
    ...
    const timer = setTimeout(() => {
      ...
      setStatus(previous => ({ ...previous, [card.cardId]: { pending: false, message: "No delegation result received. Check the CLI before trying again." } }));
    }, 20000);
    requests.current.set(requestId, { cardId: card.cardId, timer });
    try { await getCurrentWindow().emitTo("main", "amo-focus-session-command", { requestId, action, cardId: card.cardId, sessionComponentId: card.session.componentId, conversationComponentId: card.conversation!.componentId, sessionId: card.session.sessionRef.sessionId, frameworkId: card.session.sessionRef.frameworkId }); }
