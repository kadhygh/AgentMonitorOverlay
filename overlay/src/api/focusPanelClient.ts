...
export async function loadFocusCards(signal?: AbortSignal) {
  const result = await requestCardJson<{ schemaVersion: 2; cards: FocusCardView[]; groups: TaskGroup[]; groupRevision: number; reviewGroupId: string | null }>(`${FOCUS_URL}?includeArchived=1`, { signal });
  ...
  if (!(result.reviewGroupId === null || typeof result.reviewGroupId === "string" && result.groups.some(group => group.groupId === result.reviewGroupId))) throw new Error("This broker does not support the configured Review group. Start the matching AMO broker.");
  return result;
}
