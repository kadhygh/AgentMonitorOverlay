...
| Codex CLI | Verified through Codex App Server `thread/name/set`; this is available after the Hook exposes the provider thread id, not as a new-session CLI argument. | Launch plain `codex`. When an enrolled workspace has `workspaceLabel`, AMO derives a deterministic local `taskTitle` from the first `UserPromptSubmit` of a `startup` session. Provider rename happens only after an explicit user `Sync to Session` action. Resume, clear, compact, duplicate, and later prompts do not auto-name again. |
| Grok Build | No verified native rename adapter. Grok generates its own session title after the first turns. | Launch `grok`. The first `UserPromptSubmit` of a `startup` or `new` session with `workspaceLabel` receives an AMO-only local `taskTitle`. Resume, compact, duplicate, and later prompts do not rename again. If AMO has not named the card, Broker can fall back to Grok's local `generated_title`. |
...
Workspace-label naming is independent of terminal titles and target binding. Automatic naming writes only the AMO `taskTitle` and records `sessionNaming.status = amo-only`. Workspace Tools compares `taskTitle` with the current provider `title`: equal names are shown as synced, different names offer an explicit provider sync action when that provider supports rename. Provider sync attempts are recorded separately in `providerNameSync`.
