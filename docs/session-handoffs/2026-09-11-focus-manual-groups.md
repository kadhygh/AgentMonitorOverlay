...
新建、设置和详情有对话框；标题卡可以拖拽或右键移动。组内顺序按创建时间稳定排列，不因运行更新而跳动。设置可新增、改名、改变 dragOnly、删除。用户在原生体验后明确不要“未分组”，当前面板和列表仅展示用户定义的组；无引用记录保留但不展示。Card 归档/恢复在详情中提供；有未保存编辑时先保存或明确采用最新内容。
...
上述“未分组”是首次部署时的观测，随后已按用户反馈移除默认兜底展示。修正验证：17项 Focus/分组运行时用例、生产构建通过，隔离真实UI smoke `tmp/focus-manual-smoke-QMmzal/` 页面错误为空，覆盖删除组后记录保留但退出面板/列表，显式重新分配后才再次展示。

用户同时提出“归档应代表不再关注”。已完成只读 [归档成本审计](../tasks/card-archive-cost-audit-2026-09-11.md)，指出 Card 层仍观察/全量拉取归档记录，以及历史归档 Session 的默认 Card 导入问题；尚未改动这些生命周期规则。
