...
真实 Windows 透明背景、开关同步、设置弹窗此前已在生产核对。穿透、控件响应、拖拽期间捕获及结束后恢复已在隔离原生 Tauri probe 上实测，详见 [输入区域验证](tasks/focus-input-regions-2026-09-11.md)。**跨显示器/不同 DPI、与真实 CLI/Unity 的完整操作和 CLI/App 返回仍未完成。** 生产原有卡片数据保留，未配置分组的历史卡片不自动展示。
