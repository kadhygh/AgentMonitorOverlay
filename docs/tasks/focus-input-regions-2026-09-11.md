...
实现提交 `7caeac7`，尚未合入生产。自动审批拒绝了仅重启 UI 的部署命令，要求对本次生产切换另行明确确认；该命令未执行，生产仍为前一版本。待授权后只需要更新原生窗口/前端，Broker 和数据不需要变更。
...
完整生产 Focus 的穿透检查在部署后补记；跨显示器、不同 DPI、与各实际 IDE/CLI/Unity 的长时间操作仍未完成。没有把勾选前未保存的生产组属性自动改为 true，需用户重新勾选预期的组。
