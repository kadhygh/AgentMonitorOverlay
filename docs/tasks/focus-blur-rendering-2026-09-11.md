...
实现提交 `d7e211e`，用户确认“合入并重启”后，已连同交接记录 `376daf0` 快进合入本地 master，并于2026-09-11 18:02（香港时间）完成生产 Source 窗口/前端重启。Broker 保持原进程，四项既有未提交文件哈希校验通过，未 push。
...
生产原生程序部署后正常响应，启动日志达到 interactive/firstVisibleFrame。继续做生产窗口命中/截图检查时，Windows 已锁屏，屏幕命中的是 LockApp，因此未完成可见的生产失焦对比；没有操作或绕过锁屏。解锁后仍需复核实际面板。

跨DPI、系统级主题切换及最小化/恢复尚未作为本次结论；尤其Tao的系统主题更新有直接调用默认窗口过程的另一条路径，未在无证据情况下扩大改动。
